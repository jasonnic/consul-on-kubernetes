...
* [OpenShift](https://docs.openshift.com/container-platform/3.6/welcome/index.html)

/I'm running minishift start --metrics --logging --vm-driver virtualbox/ 
[CDK](https://developers.redhat.com/download-manager/file/devsuite-2.1.0-GA-bundle-installer-mac.zip)
...
```
 oc create secret generic consul  \ --from-literal="gossip-encryption-key=${GOSSIP_ENCRYPTION_KEY}" \
  --from-file=ca.pem \
...
```
oc create configmap consul --from-file=configs/server.json
```
...
```
oc create -f services/consul.yaml
```

### Create the Consul StatefulSet

Deploy a three (3) node Consul cluster using a StatefulSet:
For MiniShift this *MUST* be changed to single node mode
Change /statefulsets/consul.yaml to remove antiaffinity block


```
oc  create -f statefulsets/consul.yaml
```
...
```
oc port-forward consul-0 8400:8400
```
```
...
```
oc  port-forward consul-0 8500:8500
```
